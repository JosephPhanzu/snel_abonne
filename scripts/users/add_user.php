<?php
    ...
    try {
        ...
        $mdp1 = isset($_POST['password']) ? filter_input(INPUT_POST, securisation('password'), FILTER_SANITIZE_SPECIAL_CHARS) : null;
        $mdp2 = isset($_POST['password1']) ? filter_input(INPUT_POST, securisation('password1'), FILTER_SANITIZE_SPECIAL_CHARS) : null;
            ...
            if (preg_match('/^[0-9]{10}$/', $telephone)) {
                if (preg_match('/(@cnss.org)$/i', $email)) :
                    ...
                    if ($proprio = Employeur::add()) {
                        ...
                        $permission = new Permission('Agent',  $code_user, 1, 1);
                        if ($permission->add()) :
